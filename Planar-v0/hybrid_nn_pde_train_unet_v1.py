...
class Chemical_reaction(SpEnergy):
    ...
    def step(self, smoke, eq=1.0, dt=1.0, gravity=Gravity()):
        Q1 = 1
        wkf = Q1 * (Vf) * Wf
        wko = Q1 * (Vo) * Wo

        wt = hk * (wkf)
        smoke = smoke.copied_with(velocity = 0.0, pressure = 101325, Wkf = wkf, Wko = wko, Wt = wt)
...
st_co = SpFluid(Domain([params['res'], params['res']], box=AABox(0, [params['bx'], params['bx']]), boundaries=CLOSED), buoyancy_factor=0, batch_size=params['sbatch'])
st_gt = [ SpFluid(Domain([params['res'], params['res']], box=AABox(0, [params['bx'], params['bx']]), boundaries=CLOSED), buoyancy_factor=0, batch_size=params['sbatch']) for _ in range(params['msteps']) ]
...
for j in range(params['epochs']):  # training
    ...
    for ib in range(dataset.numOfBatchs):   # for each batch
        for i in range(dataset.numOfSteps):  # for each step
            ...
            er_nr = adata[3]  # equivalence ratio
