...
for j in range(params['epochs']):  # training
    ...
    for ib in range(dataset.numOfBatchs):   # for each batch
        for i in range(dataset.numOfSteps):  # for each step
            ...
            i_st += 1
            end1 = time.time()
            log.info('epoch {:03d}/{:03d}, batch {:03d}/{:03d}, step {:04d}/{:04d}: loss={}, time1={}'.format(
                j+1, params['epochs'], ib+1, dataset.numOfBatchs, i+1, dataset.numOfSteps, l2, (end1-start1)
            ))
            dataset.nextStep()

        dataset.nextBatch()
    #end0 = time.time()
    #log.info('epoch {:03d}/{:03d}, time1={}, gpu_used={}, gpu_free={}'.format(j+1, params['epochs'],(end0-start0), info.used, info.free))
