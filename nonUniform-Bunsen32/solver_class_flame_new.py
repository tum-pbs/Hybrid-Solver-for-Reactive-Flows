...
def VelocityBoundary_y(field, rd_array, res, rd_yk, equi_ratio):
    ...
    #vn[:, 0:1, :, 0] = amp # bottom uy
    #print(rd[0, 0:1, 0:32, 0])
    zf_conditions = get_zf_bc_vector(rd_yk, equi_ratio)
    zf_list = [0.047, 0.052]
    u_list = [10, 12, 17]

    vn[:, 0:1, 0:res, 0] = np.absolute(rd_array[:, 0:1, 0:res, 0]) * 6 + 8
    #vn[:, 0:1, res:(res + 1), 0] = np.absolute(rd_array[:, 0:1, (res-1):res, 0]) * 6 + 8
    velBCy = vn
