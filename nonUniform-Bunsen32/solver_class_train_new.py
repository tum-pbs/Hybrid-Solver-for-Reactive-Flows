...
def VelocityBoundary_y(field, rd_array, res):
    ...
    for d in range(field.data[0].data.shape[0]):
        ...
        vm0 = tf.abs(rd_array[d:(d+1),0:1,:,:]) * 6 + 8
        if d == 0:
            vm1 = vm0
        else:
...
Supports obstacles, density effects, velocity effects, global gravity.
    ...
    def step(self, fluid, eq=1.0, amp=1.0, dt=1.0, obstacles=(), gravity=Gravity(), density_effects=(), velocity_effects=()):
        ...
        cx, cy = vel[1], vel[0]
        velBCy, velBCyMask = VelocityBoundary_y(velocity, rd_array, 32)
        ...
        yf2, yo2 = tf.zeros([Yf.data.shape[0],31,32,1]), tf.zeros([Yo.data.shape[0],31,32,1])
        print(Yf.data.shape)
