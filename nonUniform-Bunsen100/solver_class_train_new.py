...
class SpFluid(DomainState):
    ...
    def __init__(self, domain, velocity=0.0, temperature=0.0, pressure=101325, rd=0.0, Yf=0.0, Yo=0.0, Wt=0.0, Wkf=0.0, Wko=0.0, buoyancy_factor=0.0, amp=1.0, eq=1.0, tags=('fluid', 'velocityfield'), name='fluid', **kwargs):
...
This force is scaled with the buoyancy_factor (float).
    ...
    def __repr__(self):
        return "Fluid[velocity: %s, temperature: %s, pressure: %s, rd: %s, Yf: %s, Yo: %s, Wt: %s, Wkf: %s, Wko: %s]" % (self.velocity, self.temperature, self.pressure, self.rd, self.Yf, self.Yo, self.Wt, self.Wkf, self.Wko)
...
def VelocityBoundary_y(field, rd_array, res):
    #vn = np.zeros(field.data[0].data.shape)  # NOTE: st.velocity.data[1] is considered as the velocity field in x axis!
    vm2 = tf.zeros([field.data[0].data.shape[0], res, res, 1])
    #v1 = tf.ones([1, 1, 32, 1])
    #print(vn.shape, amp.shape)
    for d in range(field.data[0].data.shape[0]):
        #vm0 = tf.ones([1, 1, res, 1])
        vm0 = tf.abs(rd_array[d:(d+1),0:1,:,:]) * 4 + 6
        if d == 0:
            vm1 = vm0
        else:
            vm1 = tf.concat((vm1, vm0), axis=0)

    velBCy = tf.concat((vm1, vm2), axis=1)
...
Supports obstacles, density effects, velocity effects, global gravity.
    ...
    def step(self, fluid, eq=1.0, amp=1.0, dt=1.0, obstacles=(), gravity=Gravity(), density_effects=(), velocity_effects=()):
        ...
        vel = velocity.data
        rd_array = fluid.rd.data
        cx, cy = vel[1], vel[0]
        velBCy, velBCyMask = VelocityBoundary_y(velocity, rd_array, 100)
        ...
        yf2, yo2 = tf.zeros([Yf.data.shape[0],99,100,1]), tf.zeros([Yo.data.shape[0],99,100,1])
        print(Yf.data.shape)
        Zf, Zo = get_zf_zo(fluid.eq)
        print(Zf.shape, Zo.shape)
        yf_mask[:, 0:1, :, :] = 0

        for d in range(Yf.data.shape[0]):
            yf0 = tf.ones([1, 1, 100, 1])
            yf0 = tf.multiply(Zf[d], yf0)
            if d == 0:
                yf1 = yf0
            else:
                yf1 = tf.concat((yf1, yf0), axis=0)
        yf = tf.concat((yf1, yf2), axis=1)

        yo_mask[:, 0:1, :, :] = 0
        for d in range(Yo.data.shape[0]):
            yo0 = tf.ones([1, 1, 100, 1])
            yo0 = tf.multiply(Zo[d], yo0)
            if d == 0:
                yo1 = yo0
            else:
                yo1 = tf.concat((yo1, yo0), axis=0)
        yo = tf.concat((yo1, yo2), axis=1)
        ...
        return fluid.copied_with(velocity=velocity, temperature= temperature, pressure = pressure, rd=fluid.rd, Yf = Yf, Yo = Yo, Wt = wt, Wkf = wkf, Wko = wko, age=fluid.age + dt)
