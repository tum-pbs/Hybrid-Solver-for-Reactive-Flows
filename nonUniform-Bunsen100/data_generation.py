...
rd_array = rst.rd.data
rd_array = rd_array.reshape((params['res'],params['res']))
print(rd_array.shape)

def func_s(rd_y):
    x = np.arange(len(rd_y))
    itp = interp1d(x,rd_y, kind='linear')
    window_size, poly_order = 21, 3
    rd_yy = savgol_filter(itp(x), window_size, poly_order)
    return rd_yy

rd_smooth = np.apply_along_axis(func_s, 1, np.absolute(rd_array))
rd_smooth = rd_smooth.reshape((1,params['res'],params['res'],1))
print(rd_smooth.shape)
rst = rst.copied_with(rd=rd_smooth)
...
tf_rst = simulator.step(tf_rst_in, react=1, rd_array=rd_array, res=params['res'], dt=0.0004)
