...
rst = SpFluid(Domain([params['res'], params['res']], box=AABox(0, [params['bx'], params['bx']]), boundaries=[OPEN,CLOSED]), buoyancy_factor=0, amp=params['amp'], eq=params['er'])
...
tf_rst = simulator.step(tf_rst_in, react=1, dt=0.0005)
...
for i in range(1, params['simsteps']):
    ...
    #log.info('age {:6f}'.format(rst.age))
    if params['output'] is not None:
