...
def to_feature(smokestates, ext_const_channel):
    ...
    velocity_x2 = [np.zeros([smokestates[j].velocity.data[1].data.shape[0], 32, 1, 1]) for j in range(len(smokestates))]
...
class Chemical_reaction(SpEnergy):
    ...
    def step(self, smoke, dt=1.0, gravity=Gravity()):
        Q1 = 1
        wkf = Q1 * (Vf) * Wf
        wko = Q1 * (Vo) * Wo
...
for i in range(1, params['simsteps']):
    ...
    st[-1] = simulator.step(st[-1], dt=0.0005)

    inputf = (to_feature(st, E0[0]) - [*(data_stats['mean'][0:3]), *(data_stats['mean'][4]), 0.0])/[
