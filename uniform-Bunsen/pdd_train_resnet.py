...
def to_feature(smokestate, ext_const_channel, extA_const_channel):
    with tf.name_scope('to_feature') as scope:
        return math.concat(
            [
                ...
                tf.constant(shape=smokestate.temperature.data.shape, value=1.0)*math.reshape(value=extA_const_channel, shape=[smokestate._batch_size, 1, 1, 1]),
            ],
            axis=-1
        )
...
class PhifDataset():
    def __init__(self, dirpath, num_frames, num_sims=None, batch_size=1, print_fn=print):
        ...
        self.pathsTempNR   = [sorted(glob.glob(asim + '/nr_temp_0*.npz')) for asim in self.dataSims]
        self.pathsYfNR     = [sorted(glob.glob(asim + '/nr_Yf_0*.npz')) for asim in self.dataSims]
        self.pathsYoNR     = [sorted(glob.glob(asim + '/nr_Yo_0*.npz')) for asim in self.dataSims]
        ...
        self.dataStats.update({
            'ext.mean': [
                np.mean([np.absolute(self.extConstChannelPerSim_amp[asim][i]) for asim in self.dataSims]) for i in range(num_of_ext_channel)  # equivalence ratio
            ],
            'ext.std': [
                np.std([np.absolute(self.extConstChannelPerSim_amp[asim][i]) for asim in self.dataSims]) for i in range(num_of_ext_channel)  # equivalence ratio
            ]
        })
...
with tf.name_scope('training') as scope:
    ...
    model = netModel(to_feature(tf_st_co_in, tf_st_er_in, tf_st_amp_in))
        ...
        for i in range(params['msteps']):
            with tf.name_scope('step_w_pred') as scope:
                ...
                with tf.name_scope('pred') as scope:
                    tf_cv_md += [
                        model(
                            (to_feature(tf_st_co_prd[-1], tf_st_er_in, tf_st_amp_in) - ([*(dataset.dataStats['mean'][0:3]),
                                                                                        0.0, dataset.dataStats['ext.mean'][0]])) / ([*(dataset.dataStats['std'][0:3]), 1.0, dataset.dataStats['ext.std'][0]])
...
for j in range(params['epochs']):  # training
    ...
    for ib in range(dataset.numOfBatchs):   # for each batch
        for i in range(dataset.numOfSteps):  # for each step
            ...
            er_nr, amp_nr = adata[3], adata[4] # equivalence ratio, amp
